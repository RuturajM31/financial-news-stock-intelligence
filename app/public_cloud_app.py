...
def _render_final_executive_header_v26() -> None:
    """Render the final Executive Overview header directly above input controls."""
    ...
    st.markdown(
        ...
        .final-exec-header {
            ...
            background:
                radial-gradient(circle at 0% 0%, rgba(168, 85, 247, 0.20), transparent 22rem),
                radial-gradient(circle at 100% 0%, rgba(20, 184, 166, 0.18), transparent 24rem),
                linear-gradient(135deg, rgba(3, 2, 8, 0.98), rgba(14, 10, 30, 0.88));
            box-shadow: 0 24px 70px rgba(0,0,0,.38), inset 0 1px 0 rgba(255,255,255,.045);
        }
        ...
        .final-exec-icon {
            ...
            place-items: center;
            border-radius: 16px;
            background: linear-gradient(135deg, rgba(168,85,247,.30), rgba(20,184,166,.24));
        ...
        .final-exec-badge {
            ...
            border-radius: 999px;
            border: 1px solid rgba(45,212,191,.34);
            background: linear-gradient(135deg, rgba(6,78,59,.46), rgba(88,28,135,.42));
            color: #ecfeff;
            font-size: .78rem;
            font-weight: 950;
            white-space: nowrap;
        ...
        <section class="final-exec-header">
            <div class="final-exec-kicker">Analysis Control Center</div>
            <div class="final-exec-grid">
                <div>
                    <div class="final-exec-title">
                        <span class="final-exec-icon">📈</span>
                        <span>Executive Overview</span>
                    </div>
                    <div class="final-exec-subtitle">
                        Financial News → Sentiment → Movement → Forecast → Explainability
                    </div>
                    <div class="final-exec-helper">
                        Enter a URL, upload an article, or paste text. The dashboard updates the conclusion,
                        scorecards, charts, workflow, and drivers.
                    </div>
                </div>
                <div class="final-exec-badges">
                    <div class="final-exec-badge">☁ Public Cloud</div>
                    <div class="final-exec-badge">🔗 URL Input</div>
                    <div class="final-exec-badge">⇧ Upload Input</div>
                    <div class="final-exec-badge wide">👁 Workflow Visible</div>
                </div>
            </div>
        </section>
        """,
...
def render_public_streamlit_cloud_app(project_root: Path) -> None:
    ...
    if page == "Executive Overview":
        ...
        _render_final_executive_header_v26()
